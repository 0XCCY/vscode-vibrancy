...
const themeConfigPaths = {
  ...
  "Tokyo Night": "../themes/Tokyo Night.json",
};
...
async function changeTerminalRendererType() {
  let v = vscode.workspace
    .getConfiguration()
    .inspect("terminal.integrated.gpuAcceleration");
  if (v !== undefined) {
    if (!v.globalValue) {
...
async function checkColorTheme() {
  const currentTheme = getCurrentTheme(
    vscode.workspace.getConfiguration("vscode_vibrancy")
  );
  const themeConfig = require(path.join(
    __dirname,
    themeConfigPaths[currentTheme]
  ));
  ...
  if (target !== currentColorTheme) {
    ...
    await vscode.window
      .showInformationMessage(
        message,
        localize("messages.changeColorThemeIde"),
        localize("messages.noIde")
      )
      .then(async (msg) => {
...
function activate(context) {
  ...
  async function installJS() {
    ...
    const themeCSS = await fs.readFile(
      path.join(__dirname, themeStylePaths[currentTheme]),
      "utf-8"
    );
    ...
    const newJS =
      ...
      `global.vscode_vibrancy_plugin = ${JSON.stringify(
        injectData
  ...
  async function uninstallJS() {
    const JS = await fs.readFile(JSFile, "utf-8");
    const needClean =
      /\/\* !! VSCODE-VIBRANCY-START !! \*\/[\s\S]*?\/\* !! VSCODE-VIBRANCY-END !! \*\//.test(
        JS
      );
    if (needClean) {
  ...
  function enabledRestart() {
    vscode.window
      .showInformationMessage(localize("messages.enabled"), {
        title: localize("messages.restartIde"),
      })
      .then(function (msg) {
        msg && promptRestart();
      });
  ...
  function disabledRestart() {
    vscode.window
      .showInformationMessage(localize("messages.disabled"), {
        title: localize("messages.restartIde"),
      })
      .then(function (msg) {
        msg && promptRestart();
    ...
    try {
      ...
      if (error && (error.code === "EPERM" || error.code === "EACCES")) {
        vscode.window.showInformationMessage(
          localize("messages.admin") + error
        );
      } else {
        vscode.window.showInformationMessage(
          localize("messages.smthingwrong") + error
        );
      }
      throw error;
    }
  ...
  var installVibrancy = vscode.commands.registerCommand(
  ...
  var uninstallVibrancy = vscode.commands.registerCommand(
  ...
  var updateVibrancy = vscode.commands.registerCommand(
  ...
  if (isFirstload()) {
    vscode.window
      .showInformationMessage(localize("messages.firstload"), {
  ...
  vscode.workspace.onDidChangeConfiguration(() => {
    ...
    if (!deepEqual(lastConfig, newConfig)) {
      ...
      vscode.window
        .showInformationMessage(localize("messages.configupdate"), {
